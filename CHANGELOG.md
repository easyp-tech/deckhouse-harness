...
- **`GetPodLogs` ignored its documented default-container behavior.** The proto says "if container is omitted, the default container (first container) is used", but the handler passed an empty container to the Kubernetes API, which errors for multi-container pods (`a container name must be specified for pod X, choose one of: [...]`). Since Deckhouse pods are frequently multi-container (e.g. the `deckhouse` pod is 2/2), calling `GetPodLogs` by pod name alone failed. The client now resolves the pod's first container when none is given (`client.defaultContainer`). Verified live against a two-container pod and the real 2/2 `deckhouse` pod. Unit test: `k8s.TestDefaultContainer`.
- **`GetClusterStatus.unhealthyPodsCount` was wrong.** Two problems: (1) it scanned only the `d8-system` namespace, but the proto contract is "across all namespaces" — so unhealthy pods anywhere else were not counted; (2) it used a phase-only `isPodHealthy` check, inconsistent with `ListUnhealthyPods`. It now lists pods in all namespaces and reuses the same container-aware `toUnhealthyPodInfo` check, so a CrashLoopBackOff/ImagePullBackOff pod is counted here too. The dead `isPodHealthy` helper was removed. Verified on a live cluster (a broken MongoDB pod in another namespace: count went 0 → 1, matching `kubectl`). Unit test: `GetClusterStatus_UnhealthyCountAllNamespaces`.
- **`ListUnhealthyPods` missed CrashLoopBackOff / ImagePullBackOff pods.** These pods report `status.phase=Running` (a container is in a waiting state), so the old phase-only filter skipped them — the single most common unhealthy case. The handler now also inspects container statuses (init + regular): a pod is reported when any container is waiting with a problem reason (CrashLoopBackOff, ImagePullBackOff, ErrImagePull, CreateContainerError, …) or terminated with a non-zero exit code, and the reported `status` is the container reason (e.g. `CrashLoopBackOff`) rather than the bare pod phase. Found by running a real crashing pod on a live Deckhouse cluster. Unit test: `ListUnhealthyPods_CrashLoopBackOff`.
