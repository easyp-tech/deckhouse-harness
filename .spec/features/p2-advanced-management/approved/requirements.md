...
Не конфликт — разная гранулярность. CordonNode остаётся отдельным tool для сценариев,
когда drain не нужен (плановая разгрузка, dry-run, ручное обслуживание).

Резолюция дизайна (передаётся в фазу Design):

1. Cordon-логика реализуется на уровне k8s.Client (метод CordonNode(ctx, name)).
   DrainNode НЕ вызывает MCP tool CordonNode — оба handler'а переиспользуют
   один и тот же метод k8s.Client. Это устраняет дублирование и упрощает
   мокирование в тестах.

2. Шаг cordon внутри DrainNode идемпотентен: если нода уже cordoned,
   шаг считается успешным, операция продолжается с eviction.

3. При ошибке на шаге eviction (включая истечение DrainTimeout) cordon
   НЕ откатывается. Поведение совпадает с `kubectl drain`: оператор
   может снять cordon отдельным вызовом UncordonNode после расследования.
   Автоматический откат опасен — на нестабильную ноду немедленно
   возвращаются новые поды.

4. Возвращаемые поля:
   - CordonNode / UncordonNode: previousState bool (было ли уже cordoned).
   - DrainNode: cordoned bool, evictedCount int, failedPods []string,
     timedOut bool. Поле previousState не нужно — drain всегда оставляет
     ноду cordoned.
```
